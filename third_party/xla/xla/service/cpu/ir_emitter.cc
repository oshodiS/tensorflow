...
void EmitTransferElements(llvm::Value* target, llvm::Value* source,
  ...
  // TODO(paruzelp): this function does not really depend on llvm::Module but
  //                 rather llvm::LLVMContext, however PrimitiveTypeToIrType
  //                 requires llvm::Module (even though it only uses
  //                 llvm::LLVMContext). Probably could refactor this.
  unsigned primitive_type_size =
      ShapeUtil::ByteSizeOfPrimitiveType(primitive_type);
  llvm::Align element_alignment(tsl::MathUtil::GCD<unsigned>(
      primitive_type_size,

...
class ReductionRewriterVisitor : public DfsHloRewriteVisitor {
 public:
  ...
  absl::Status HandleReduce(HloInstruction *hlo) override {
    ...
    if (!MatchReductionForSplit(reduce, reduction_dims)) {
 ...
 private:
  bool MatchReductionForSplit(HloReduceInstruction *reduce,
                              const ReductionDimensions &reduction_dims) {
  ...
  absl::Status SplitReductionDimension(
    ...
    *split_dim_it += 1;
    ...
    for (int dim_idx = 0, reduced_dim_idx = 0;
         dim_idx < reshaped_dimensions.size(); ++dim_idx) {
      if (reduced_dim_idx < inner_reduce_dims.size() &&
          inner_reduce_dims[reduced_dim_idx] == dim_idx) {
        ++reduced_dim_idx;
        continue;
      }
      inner_reduce_shape.push_back(reshaped_dimensions[dim_idx]);
    }
